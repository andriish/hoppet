  ...
  subroutine holder_InitSigma(gd, sh, factscheme, nloop, nflo, nfhi)
    ...
    do nflcl = lbound(sh%allP,dim=2), ubound(sh%allP,dim=2)
       ...
       case (factscheme_MSbar)
          call InitSplitMatLO (gd, sh%P)
          if (sh%nloop >= 2) call InitSplitMatNLO(gd, sh%P1, sh%factscheme)
          if (sh%nloop >= 3) then
             call InitSplitMatNNLO(gd, sh%P2, sh%factscheme)
             !-- do this once, and only if really needed
             if (lbound(sh%allP,dim=2) /= ubound(sh%allP,dim=2) &
                  &.and. mass_steps_on &
                  &.and. nflcl == lbound(sh%allP,dim=2)) then
                call InitMTMNNLO(gd,sh%MTM2)
                sh%MTM2_exists = .true.
             end if
          end if
       ...
       case (factscheme_DIS)
          call InitSplitMatLO (gd, sh%P)
          if (sh%nloop >= 2) &
               &call InitSplitMatNLO(gd, sh%P1, factscheme_MSbar)
          ...
          if (newDIS) then
             ...
             !           \ -Cq  -Cg /
             call InitGridConv(gd,CC(iflv_sigma,iflv_sigma),cf_CqF2MSbar)
             call InitGridConv(gd,CC(iflv_sigma,iflv_g),    cf_CgF2MSbar)
             ...
             !call SetToCommutator(tmp2d,CC,sh%P%singlet(iflv_g:iflv_sigma,iflv_g:iflv_sigma))
             call SetToCommutator(tmp2d,CC,sh%P%singlet)
             !-- add it to P1
             call AddWithCoeff(sh%P1%singlet,tmp2d)
             !-- add the beta function pieces as well
             call AddWithCoeff(sh%P1%singlet,CC, -twopi_beta0)
             call AddWithCoeff(sh%P1%NS_plus, CC(iflv_sigma,iflv_sigma),&
                  & -twopi_beta0)
             !-- quark number conservation remains OK because
             !   Cq has the om=0 moment equal to zero.
             call AddWithCoeff(sh%P1%NS_minus, CC(iflv_sigma,iflv_sigma), &
                  &-twopi_beta0)
             call AddWithCoeff(sh%P1%NS_V, CC(iflv_sigma,iflv_sigma), &
                  &-twopi_beta0)
             ...
             call InitGridConv(gd, Cq, cf_CqF2MSbar)
             call InitGridConv(gd, Cg, cf_CgF2MSbar)
             ...
             call AddWithCoeff(sh%P1%qq, CgPgq, one)
             call AddWithCoeff(sh%P1%qq, CqPqg, one)
             !   Pqg -> Pqg + (Cq Pqg + Cg Pgg - Cg Pqq + Cg Pqg)
             call AddWithCoeff(sh%P1%qg, CqPqg, one)
             call AddWithCoeff(sh%P1%qg, CgPgg, one)
             call AddWithCoeff(sh%P1%qg, CgPqq,-one)
             call AddWithCoeff(sh%P1%qg, CgPqg, one)
             !   Pgq -> Pqg + (-Cq Pqq - Cg Pgq - Cq Pgq + Cq Pgg)
             call AddWithCoeff(sh%P1%gq, CqPqq,-one)
             call AddWithCoeff(sh%P1%gq, CgPgq,-one)
             call AddWithCoeff(sh%P1%gq, CqPgq,-one)
             call AddWithCoeff(sh%P1%gq, CqPgg, one)
             !   Pgg -> Pgg + (-Cq Pqg - Cg Pgq)
             call AddWithCoeff(sh%P1%gg, CqPqg,-one)
             call AddWithCoeff(sh%P1%gg, CgPgq,-one)
             !
             !   Now deal with P_matrix -> P_matrix - beta0 * C
             !                 P_+      -> P_+      - beta0 * C_q
             call AddWithCoeff(sh%P1%qq, Cq, -twopi_beta0)
             call AddWithCoeff(sh%P1%qg, Cg, -twopi_beta0)
             call AddWithCoeff(sh%P1%gq, Cq, +twopi_beta0)
             call AddWithCoeff(sh%P1%gg, Cg, +twopi_beta0)
             call AddWithCoeff(sh%P1%NS_plus, Cq, -twopi_beta0)
             !-- quark number conservation remains OK because
             !   Cq has the om=0 moment equal to zero.
             call AddWithCoeff(sh%P1%NS_minus, Cq, -twopi_beta0)
             call AddWithCoeff(sh%P1%NS_V, Cq, -twopi_beta0)
             !
             !   tidy up
             ...
             call Delete(CgPgq)
             call Delete(CgPgg)
             write(0,*) 'result:',sh%P1%singlet(iflv_sigma,iflv_sigma)%conv(10,1)
          end if
          
       case (factscheme_PolMSbar)
          write(0,*) "SETTING UP POLARIZED EVOLUTION"
          call InitSplitMatPolLO (gd, sh%P)
          if (sh%nloop >= 2) call InitSplitMatPolNLO(gd, &
               &sh%P1, sh%factscheme)
