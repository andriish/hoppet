...
contains
  ...
  !! improvement here!
  !!
  subroutine pdftab_AllocTab_(gd, tab, Qmin, Qmax, dlnlnQ, freeze_at_Qmin, lnlnQ_order)
    ...
    call AllocPDF(gd,tab%tab,0,tab%nQ)
  ...
  subroutine pdftab_AssocNfInfo(tab,ash)
    ...
    call AllocPDF(tab%gd,tab%tab,0,tab%nQ)
    allocate(tab%lnlnQ_vals(0:tab%nQ))
  ...
  subroutine pdftab_InitTabSub_(tab, sub)
    ...
    do iQ = 0, tab%nQ
       ...
       call InitPDFSub(tab%gd, tab%tab(:,:,iQ), sub, Q)
    end do
  ...
  subroutine pdftab_InitTabSub_iset(tab, sub, iset)
    ...
    do iQ = 0, tab%nQ
       ...
       call InitPDFSub(tab%gd, tab%tab(:,:,iQ), sub, Q, iset)
    end do
  ...
  subroutine pdftab_InitTab_LHAPDF(tab, LHAsub)
    ...
    do iQ = 0, tab%nQ
       Q = invlnln(tab,tab%lnlnQ_min + iQ*tab%dlnlnQ)
       call InitPDF_LHAPDF(tab%gd, tab%tab(:,:,iQ), LHAsub, Q)
    end do
  ...
  subroutine pdftab_ValTab_yQ(tab,y,Q,val)
    ...
    call WgtGridQuant(tab%gd, y, iylo, y_wgts)
