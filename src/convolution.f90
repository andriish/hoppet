...
module convolution
  ...
  public :: SetConvolutionEps, GridConvAllocated
...
contains
  ...
  subroutine conv_InitGridDef_single(gd,dy,ymax,order)
    ...
    gd%ny   = nint(ymax / dy)
    if (gd%ny < 2) then
       write(0,*) 'InitGridDef: requested too small a number of bins'
    ...
    if (abs(gd%dy/dy - one) > 0.001_dp) then
       write(0,*) 'InitGridDef: requested dy of', dy
    ...
    if (present(order)) then
       ...
       if (abs(order)+1 > gd%ny) then
          write(0,*) &
            &'ERROR in InitGridDef: |order|+1 > number of grid points' 
       end if
       gd%order = order
    else
       gd%order = LIN_ORDER
    ...
    if (gd%locked) then
       ...
       do i = gd%nsub-1, 1, -1
          ...
          if (abs(dyratio-nint(dyratio)) > warn_tolerance*dyratio) then
             write(0,'(a,i2,a,f18.14)') ' InitGridDef (locking):&
          ...
          if (abs(subgd(i)%order)+1 > subgd(i)%ny) then
             write(0,'(a)') 'Error in InitGridDef (locking):'
  ...
  recursive function yValues(gd) result(res)
    ...
    if (gd%nsub /= 0) then
       do isub = 1, gd%nsub
          res(gd%subiy(isub):gd%subiy(isub+1)-1) = yValues(gd%subgd(isub))
       end do
    else
       forall(iy=0:gd%ny) res(iy) = iy*gd%dy
    end if
  ...
  function xValues(gd) result(res)
    type(grid_def), intent(in) :: gd
    real(dp)                   :: res(0:gd%ny)
  ...
  !! interface.
  recursive subroutine InitGridQuantLHAPDF(gd, gq, LHAsub, Q)
    real(dp),         intent(inout) :: gq(0:,:)
    type(grid_def),   intent(in)    :: gd
    real(dp),         intent(in)    :: Q
    interface
    ...
    if (gd%nsub /= 0) then
       do isub = 1, gd%nsub
          call InitGridQuantLHAPDF(gd%subgd(isub), &
...
!O  end function conv_EvalfGridQuant_1d
  ...
  recursive subroutine WgtGridQuant(gd, y, iylo, wgts)
    use interpolation
    type(grid_def), intent(in) :: gd
    real(dp), intent(in)  :: y
    ...
    if (gd%nsub /= 0) then
       isub = conv_BestIsub(gd,y)
       call WgtGridQuant(gd%subgd(isub), y, iylo, wgts)
  ...
  function MomGridQuant(gd,gq,omega) result(res)
    type(grid_def), intent(in) :: gd
    real(dp), intent(in) :: gq(0:)
    ...
    ny = assert_eq(gd%ny,ubound(gq,dim=1),"MomGridQuant")
    dy = gd%dy
    if (omega == zero) then
       weight = gd%dy
       weightprod = one
  ...
  subroutine conv_PrintGridQuant_1(gd,gq,dy,dev)
    ...
    ny = assert_eq(gd%ny,ubound(gq,dim=1),'PrintGridQuant')
    if (gd%nsub /= 0) then
  ...
  subroutine SetConvolutionEps(eps_in)
    real(dp), intent(in) :: eps_in
    eps = eps_in
  end subroutine SetConvolutionEps
  ...
  recursive subroutine conv_InitGridConv_zero(gd,gc,alloc)
    ...
    if (default_or_opt(.not.GridConvAllocated(gc),alloc)) then
       call AllocGridConv(gd,gc)
    else
       call ValidateGD(gd, gc%gd, 'conv_InitGridConv_zero')
    end if
  ...
  recursive subroutine conv_InitGridConv_gc(gc,gcin,fact,alloc)
    ...
    if (default_or_opt(.not.GridConvAllocated(gc),alloc)) then
       call AllocGridConv(gcin%gd,gc)
    else
       call ValidateGD(gcin%gd,gc%gd,'conv_InitGridConv_gc')
    end if
  ...
  subroutine conv_InitGridConv_func(gd,gc,func,alloc)
    ...
    !if (default_or_opt(.not.GridConvAllocated(gc),alloc)) call conv_InitGridConv_zero(gd,gc)
    call conv_InitGridConv_zero(gd,gc)
  ...
  subroutine conv_InitGridConv_conv(gc,gca,gcb,alloc)
    ...
    if (default_or_opt(.not.GridConvAllocated(gc),alloc)) then
       call AllocGridConv(gca%gd,gc)
    else
       call ValidateGD(gc%gd, gca%gd,&
  ...
  recursive subroutine conv_DelGridConv_0d(gc)
    ...
    if (gc%gd%nsub /= 0) then
       do isub = 1, gc%gd%nsub
          call Delete(gc%subgc(isub))
       end do
       deallocate(gc%subgc,stat=istat)
    else
       deallocate(gc%conv,stat=istat)
       if (istat /= 0) then
          write(0,*) 'ERROR: problems with deallocation in Delete'
  ...
  recursive subroutine conv_AddGridConv_gc(gc,gcadd,fact)
    ...
    call ValidateGD(gc%gd, gcadd%gd, 'conv_AddGridConv_gc')
    if (gc%gd%nsub /= 0) then
       do isub = 1, gc%gd%nsub
...
!!$       end do
  ...
  recursive subroutine conv_ConvGridConv_0d(gc,gca, gcb, reorder)
    ...
    call ValidateGD(gca%gd, gcb%gd, 'conv_ConvGridConv_0d: gca and gcb')
    if (.not.GridConvAllocated(gc)) then
       call AllocGridConv(gca%gd,gc)
    else
       call ValidateGD(gc%gd, gca%gd, 'conv_ConvGridConv_0d: gc and gca')
    end if
  ...
  subroutine conv_ConvGridConv_2dx2d(gc,gca, gcb)
    ...
    call InitGridConv(gca(1,1)%gd, gc) 
    !call SetToZero(gc)

    call AllocGridConv(gc(1,1)%gd,conv)
    do ic = 1, nc
       do ir = 1, nr
          do im = 1, nm
  ...
  !
  subroutine SetToCommutator(gc,gca, gcb)
    type(grid_conv),  intent(inout) :: gc(:,:)
    type(grid_conv),  intent(in)    :: gca(:,:), gcb(:,:)
    !----------------------------------------------------
    ...
    call AllocGridConv(gc(1,1)%gd,cc)
...
!  [a21 b11 + a22 b21 - b21 a11 - b22 a21          a21 b12 - a12 b21          ]
       call AllocGridConv(gc(1,1)%gd,prod)
    ...
    call Delete(cc)
  end subroutine SetToCommutator
  ...
  function conv_EvalGridQuant_atx(gq, gdv) result(res)
    ...
    real(dp) :: res
    res = EvalGridQuant(gdv%gd, gq, -log(gdv%val))
  ...
  function conv_EvalGridQuant_aty(gq, gdv) result(res)
    ...
    type(gdval), intent(in) :: gdv
    real(dp) :: res
    res = EvalGridQuant(gdv%gd, gq, gdv%val)
  ...
  subroutine GetDerivedProbes(gd,probes)
    type(grid_def), intent(in) :: gd
    ...
    call AllocGridQuant(gd,probes,1,nprobes)
